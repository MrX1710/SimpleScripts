...
#i'm going to try making a Payment handling (just saw it in a website (CodeForge), and based
...
#okay so if i'm not wrong: the Payment sys got is a 3 layer sys (obviously it variates idk everyhting on that but it does works also through theses steps) 
...
#i discovered that floats are dangerous in the case of Payment systems :), 0.3 + 0.1 isn't 0.4 but 0.40000000000000004 (binary float-point arithmetic operation)
...
class PaymentSys:
    ...
    def CreateUser(self, name:str, initial_balance: Decimal = Decimal('0')) -> Users: #return's Users object
    ...
    def PaymentOperations(self, sender_id:int, receiver_id:int, amount:Decimal) -> Transactions:
...
class PaymentMethod(ABC): #could be a wallet or a card
    def __init__(self, owner, processor:PaymentSys):
        self.owner = owner
        self.processor = processor #or the engine or the sys

    @abstractmethod
    def pay(self, receiver: Users, amount: Decimal) -> Transactions:
        pass
...
class Wallet(PaymentMethod):
    def __init__(self, owner:Users, processor:PaymentSys):
        super().__init__(owner, processor)

    def pay(self, receiver: "Wallet", amount:Decimal) -> Transactions:
        return self.processor.PaymentOperations(self.owner.uid, receiver.owner.uid, amount)
        #in this function, you're supposed to implement
        #you're supposed your wallet's payment rules
        #how your wallet handles Payment
        #i literally know nothing about rules that should be implemented
        #and how it should be handled
        #so i just use the Engine's Payment above as it's payment design

    def deposit(self, amount: Decimal):
        if amount <= 0:
            raise ValueError("Deposit amount must be positive and non-zero.")
        self.owner.balance += amount
        tx = Transactions(self.processor._next_tx, self.owner.uid, self.owner.uid, amount, TransactionStatus.SUCCESS)
        self.processor._transactions.append(tx)
        self.processor._next_tx += 1
        return tx
    
    def withdraw(self, amount:Decimal):
        if amount <= 0:
            raise ValueError("Your deposit must be a positive and non-zero")
        elif amount > self.owner.balance:
    ...
    def wallet_info(self) -> str:
        return f"Wallet owner: {self.owner.name} | Wallet balance: {self.owner.balance}"


class CreditCard(PaymentMethod):
    pass
class AnyPaymentMethod(PaymentMethod):
    pass